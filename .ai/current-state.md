...
- `git` disponible (Git for Windows, no en PATH): `C:\Users\HP\AppData\Local\Programs\Git\cmd\git.exe`.
...
| Smoke test DB | `npx vitest run` | **2/2 PASS** (singleton + `Transaction.create` UUID v7) |

## Stack instalado
Core (next, react 19, typescript 5.9, zod 4.4.3, eslint 9 + config-next, prettier); Data (prisma 7.9.1,
@prisma/client 7.9.1, @prisma/adapter-pg, pg); Test (vitest 4.1.10, vitest.config.mts); DevOps (dotenv, prisma.config.ts).
Pendientes por instalar: `@supabase/supabase-js` + `@supabase/ssr` (auth), `dinero.js` (Money).
...
src/
 └─ infra/
      ...
      └─ __tests__/prisma.smoke.test.ts   # 2 tests (validan DB + conexión + v7 id)
prisma/
 ├─ schema.prisma                  # 6 modelos, 5 enums, FKs, índices
...
.ai/                               # memoria canónica del agente (6 .md)
...
- `firecrawl-developer-index` INSTALAR — activa + verificada (caso Prisma 7 breaking + PrismaPg + uuid(7)).
...
2. Money: instalar `dinero.js` v2; implementar `shared/Money` (bigint) + tests Vitest.
3. Engine de dominio: módulos `transactions` (doble entrada), `accounts`, `budgets`, `goals`,
...
1. ✅ Aprobar/commit bootstrap (git disponible).
2. Money (Dinero.js v2) + tests → base del Financial Engine.
3. transactions engine (doble entrada) + tests.
