"""add_payment_reversal_and_overrides

Revision ID: a1b2c3d4e5f6
Revises: d0431b0b55d0
Create Date: 2026-08-24

"""
from alembic import op
import sqlalchemy as sa

revision = 'a1b2c3d4e5f6'
down_revision = 'd0431b0b55d0'
branch_labels = None
depends_on = None


def upgrade() -> None:
    op.add_column('debt_payments', sa.Column('is_reversed', sa.Boolean(), nullable=False, server_default='0'))

    op.create_table(
        'debt_payment_overrides',
        sa.Column('id', sa.String(36), primary_key=True),
        sa.Column('debt_id', sa.String(36), sa.ForeignKey('debts.id'), nullable=False),
        sa.Column('year', sa.Integer(), nullable=False),
        sa.Column('month', sa.Integer(), nullable=False),
        sa.Column('is_paid', sa.Boolean(), nullable=False, server_default='1'),
        sa.Column('marked_by', sa.String(36), sa.ForeignKey('users.id'), nullable=True),
        sa.Column('marked_at', sa.DateTime(), nullable=True),
    )


def downgrade() -> None:
    op.drop_table('debt_payment_overrides')
    op.drop_column('debt_payments', 'is_reversed')
