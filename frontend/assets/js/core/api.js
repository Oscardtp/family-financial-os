...
class ApiClient {
    ...
    async createQuickExpense(data) {
        return this.request('POST', '/api/v1/transactions/quick', data);
    }
    ...
    async getMonthlyReport(year, month) {
        return this.request('GET', `/api/v1/reports/monthly/${year}/${month}`);
    }

    async getExpensesByCategory(from, to) {
        const params = new URLSearchParams({ from, to }).toString();
        return this.request('GET', `/api/v1/reports/expenses-by-category?${params}`);
    }

    async getIncomeVsExpenses(from, to) {
        const params = new URLSearchParams({ from, to }).toString();
        return this.request('GET', `/api/v1/reports/income-vs-expenses?${params}`);
    }

    async getNetWorthReport(from, to) {
        const params = new URLSearchParams({ from, to }).toString();
        return this.request('GET', `/api/v1/reports/net-worth?${params}`);
    }

    // Export
    async exportTransactions(format = 'json') {
        return this.request('GET', `/api/v1/export/transactions?format=${format}`);
    }

    async exportAccounts(format = 'json') {
        return this.request('GET', `/api/v1/export/accounts?format=${format}`);
    }

    async exportAll(format = 'json') {
        return this.request('GET', `/api/v1/export/all?format=${format}`);
    }
}
